...
- Choose **Live screen capture**, **Load PDF**, **Load image**, or **Civil Plan Digitizer**.
...
Historical v2.5/v2.6 fixture results are retained for comparison. The v2.7 proof uses the production cursor-candidate path, fail-closed spatial consensus, and cache-disabled real Tesseract; its current measured result and limits are recorded in [Accuracy and validation](docs/ACCURACY.md). Fixture evidence is not a promise for an arbitrary viewer or drawing.
...
For a release build, download `Screen2XYZ-v2.7.x-windows-x64.zip`, verify it against `SHA256SUMS.txt`, unzip the entire folder, and run `Screen2XYZ-Setup\Screen2XYZ.exe`. Archive size may vary with the pinned Python and dependency builds.
...
Yes. Follow [packaging/README.md](packaging/README.md). Release tags matching `v2.7.*` run the full test/proof matrix, build the folder, create a ZIP and checksum, and publish a GitHub Release.
