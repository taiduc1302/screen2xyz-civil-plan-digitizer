...
The frozen Civil suite is currently **396 discovered tests** on `task/plan-layer-extraction` (220 at CI run #171; 255 after `bluebeam_bridge.py`; 322 after `plan_layers.py`, `earthwork.py` and the twelve added rules; 373 after `sheet_pass.py` and `cross_sections.py`; 378 with the pixel-area cross-check; 396 with `fill_layers.py`). Green locally in `.venv-operator`; not re-run in CI. Synthetic/protocol success still does **not** prove Example Road accuracy on the estimator's Revu installation.

### Since 2026-09-03: what a takeoff session now calls instead of re-deriving

- `fill_layers.split_pavement(pdf, page_index, viewport)` - the grey fill and the hatch traced as *regions* (paved works / widening / mill and overlay) with raw-frame polygons, two independent areas each, and an overlay to check. Validated on sheet 05 against the host polygons to 0.3% / 0.6%. It measured the Example Road junction intersection on sheet 04 (1 326 sq m, previously UNSEARCHED) and the Example Avenue ends on sheet 06, and found the sheet 04 clean-segment host polygons overlapping each other by 46 sq m - `docs/integrations/FILL_LAYER_EXTRACTION.md`.
