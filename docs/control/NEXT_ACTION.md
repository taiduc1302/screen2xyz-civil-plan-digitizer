...
The frozen Civil suite is currently **236 discovered tests**. A fully green CI run on the current final head is required before the owner-machine pilot is treated as code-ready. Synthetic/protocol success still does **not** prove King Road accuracy or native Bluebeam measurement creation on the estimator's Revu installation.

## Exactly one recommended next action

After current-head CI is green, run a private owner-machine acceptance on the actual King Road Sheet 03 before merging PR #8 or generalizing the architecture.
