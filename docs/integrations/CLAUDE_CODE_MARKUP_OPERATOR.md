...
For Example Road Sheet 03, use the pilot only after the estimator confirms the plan scale and independently checks it against an authoritative dimension in Revu.
...
The frozen Civil suite is **220 discovered tests**. The separate Windows protocol lane also launches the same external stdio Screen2XYZ process a real MCP host uses and a real OpenTakeoff 0.9.68 process against owned synthetic PDFs.
