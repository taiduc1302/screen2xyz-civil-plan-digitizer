...
For King Road Sheet 03, use the pilot only after the estimator has confirmed the relevant plan scale and independently checked it against a known dimension in Bluebeam/Revu.
...
.\.venv\Scripts\python.exe -m screen2xyz_civil agent-init `
  "C:\Tenders\KingRoad\IssuedForTender.pdf" `
  --page 3 `
  --page-label 03 `
