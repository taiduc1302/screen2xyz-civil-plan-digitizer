...
Bluebeam MCP payloads: `properties` **and** `shape` are arrays of `{key, value}` objects, never dicts - a dict passes validation and fails in the host with a bare "An error occurred" while reads keep working. A Polygon needs `intent: "Area"` at creation; setting it afterwards gives no live area. `pageNumber` is 1-based.
